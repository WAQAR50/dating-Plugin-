...
class Ptds_expired_events_Table extends WP_List_Table {
      ...
      //echo '<pre>'; print_r( $events);
        ...
        $ptds_activate_dating_form = '<div class="activate-dating"><a href="#" class="activate-dating-class btn btn-primary btn-info btn-sm waves-effect waves-light">
            ...
            <div class="custom-control custom-radio  mx-1">
                <input type="radio" class="custom-control-input" id="ptds_gender_male_'.$event->ID.'" name="ptds_gender" value="ptds_gender_male" required '.$ptds_checked_gender.'>
                <label class="custom-control-label" for="ptds_gender_male_'.$event->ID.'">Male</label>
            </div>
            <div class="custom-control custom-radio  mx-1">
                <input type="radio" class="custom-control-input" id="ptds_gender_female_'.$event->ID.'" name="ptds_gender" value="ptds_gender_female"  required '.$ptds_checked_gender.'>
                <label class="custom-control-label" for="ptds_gender_female_'.$event->ID.'">Female</label>
            </div>
            <div class="custom-control custom-radio ml-1">
                <input type="radio" class="custom-control-input" id="ptds_gender_both_'.$event->ID.'" name="ptds_gender" value="ptds_gender_both"  required '.$ptds_checked_gender.'>
            ...
            if(($dating_meta_event_status != 'ptdsdatingclose') && ($ptds_dating_system_ready == 'ptds_dating_system_ready') && ($dating_meta_event_status  != 'extendeddatingsystem') && ($dating_meta_event_status  != 'activatedatingsystem')){
                    $array_test[$index]['ID'] = $event->ID;
                    $array_test[$index]['post_title'] = $event->post_title;
                    $array_test[$index]['EventEndDate'] = $event->EventEndDate;
...
*/  
    ...
    // filter the table data based on the search key
     ...
     $wp_list_table_expired = new Ptds_expired_events_Table(); 
   
?>


<div class="wrap">    
    <h2><?php _e( 'Expired Events/ Active Dating System', 'ptds'); ?></h2>
		
		
			<?php 
                $wp_list_table_expired->prepare_items(); 
				$wp_list_table_expired->search_box( __( 'Seach Listing','ptds'), 'pncdatingsystem');
				$wp_list_table_expired->display(); 
			?>					
