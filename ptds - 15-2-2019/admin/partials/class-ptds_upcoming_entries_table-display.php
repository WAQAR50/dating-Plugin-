  ...
  // WHERE 1=1 AND wo_posts.post_type = 'tribe_events' ORDER BY wo_posts.post_date DESC";
      ...
      //echo '<pre>'; print_r( $events);
        ...
        foreach($events as $index => $event){
            ...
            if(($dating_meta_event_status != 'ptdsdatingclose') && ($ptds_dating_system_ready == 'ptds_dating_system_ready') && ($dating_meta_event_status != 'extendeddatingsystem')){
              
            $array_test[$index]['ID'] = $event->ID;
            $array_test[$index]['post_title'] = $event->post_title;
            $array_test[$index]['EventStartDate'] = date('m-d-Y', strtotime($event->EventStartDate));
...
*/
  ...
  $wp_list_table_upcoming = new Ptds_Upcoming_ENTRIES_Table();  
   
?>


<div class="wrap">    
    <h2><?php _e( 'Upcoming Events', 'ptds'); ?></h2>
		
		
			<?php 
                $wp_list_table_upcoming->prepare_items(); 
				$wp_list_table_upcoming->search_box( __( 'Seach Listing','ptds'), 'pncdatingsystem');
				$wp_list_table_upcoming->display(); 
			?>					
