...
class Ptds_Admin {
	...
	public function add_options_page() {
		...
		add_submenu_page(
				null,
				 __( 'Add Attendees', 'ptds' ),
				__( 'Add Attendees', 'ptds' ),
				  'manage_options', 
				'ptdsaddattendees',
			array( $this, 'display_add_attendees_page')
		);
	}
	...
	/**
	 * Render the attendees
	 *
	 * @since  1.0.0
	 */
	public function display_attendees_page() {
		include_once 'partials/class-ptds_dating_event_attendees_list.php';
	 ...
	 * @since  1.0.0
	 */
	public function display_add_attendees_page() {
		include_once 'partials/class-ptds_dating_add_attendees_page.php';
 ...
 public function ptds_datingemailcheck(){
	 	$ptdsemails = $_GET['ptdsemail'];
		$existsemail = email_exists( $ptdsemails );
		...
		if ( !empty($user_id) ) {
			...
			$response->first_name = get_user_meta($user_id, 'first_name', false);
			$response->last_name = get_user_meta($user_id, 'last_name',false);
			$response->gender = get_user_meta($user_id, 'ptdsgender', false);
		}
		else {
