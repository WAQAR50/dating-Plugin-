...
class Ptds_dating_activated_Table extends WP_List_Table {
  ...
  // WHERE 1=1 AND wo_posts.post_type = 'tribe_events' ORDER BY wo_posts.post_date DESC";
       ...
       //echo '<pre>'; print_r( $results);
        ...
        foreach($events as $index => $event){
            ...
            $ptdsdating_meta_event_date = get_post_meta( $event->ID, 'dating_meta_status_date', true );
            ...
            //Convert to date
            //$ptdsdatingeventdatestr = $ptdsdating_meta_event_date;//Your date
            $ptdsdatingeventtimestr =strtotime($ptdsdating_meta_event_date);//Converted to a PHP date (a second count)

            //Calculate difference
            $ptdsdiff = $ptdsdatingeventtimestr-time("Y-m-d");//time returns current time in seconds
            $ptdsdays = floor($ptdsdiff/(60*60*24));//seconds/minute*minutes/hour*hours/day)
            $ptdshours = round(($ptdsdiff-$ptdsdays*60*60*24)/(60*60));
            
            //Report
            //echo "$days days $hours hours remain<br />";
           if($ptdsdays >=0 && $ptdsdays <= 2){

               $ptdsactivehtml = '<div class="font-weight-bold pink-lighter-hover">'.$ptdsdays.' Days left</div>';

            }elseif($ptdsdays >=3 && $ptdsdays <=6){
                   $ptdsactivehtml = '<div class="font-weight-bold amber-lighter-hover">'.$ptdsdays.' Days left</div>';
            }else{
                $ptdsactivehtml = '<div class="font-weight-bold success-lighter-hover">'.$ptdsdays.' Days left</div>';
            }
            ...
            Extend Dating</a>    <form action="" method="" class="extend-dating-form  text-right" validate>
            <input type="number" name="extenddatingdays"  class="extenddatingdays form-control " value="10" min="1" max="100" required>
            ...
            <button type="submit" class="extend-dating-form-save btn btn-primary btn-success btn-sm text-right ">Save</button>
             ...
             $event_related_product_id = $wpdb->get_var("SELECT post_id FROM $wpdb->postmeta WHERE meta_key = '_tribe_wooticket_for_event' and meta_value = $event->ID");

           
            if(($dating_meta_event_status == 'extendeddatingsystem') || ($dating_meta_event_status == 'activatedatingsystem') && ($ptds_dating_system_ready == 'ptds_dating_system_ready')  ){
                ...
                $array_test[$index]['activate'] = $ptdsactivehtml;
                $array_test[$index]['extend'] = $ptds_extend_dating_form;
                $array_test[$index]['product_id'] = $event_related_product_id;
                $array_test[$index]['direct'] = '<a href="#" id="'.$event->ID.'" class="ptds_dating_close waves-effect btn btn-primary btn-danger btn-sm waves-light" >Direct Close</a>';
            }
        }
    ...
    function column_id($item) {
        $actions = array(
            'edit'  => sprintf('<a href="post.php?post='.$item['ID'].'&action=edit">Edit</a>',$_REQUEST['post'],'edit',$item['post_title']),
            'orders' => sprintf('<a class="deleteEntry" href="?page=ptdsorders&product_id='.$item['product_id'].'">' . esc_attr__('Orders', 'ptds') . '</a>'),
            'attendees' => sprintf('<a class="deleteEntry" href="?page=ptdsattendees&event_id='.$item['ID'].'">' . esc_attr__('Attendees', 'ptds') . '</a>'),
            'addattendees' => sprintf('<a class="deleteEntry" href="?page=ptdsaddattendees&event_id='.$item['ID'].'">' . esc_attr__('Add Attendees', 'ptds') . '</a>'),
        );
        return sprintf(' <span >%2$s</span>%3$s', '', $item['post_title'], $this->row_actions($actions)
        );
    }
   ...
   /* function column_cb($item) {
    ...
    function get_columns() {
        $columns = array(
            ...
            'EventEndDate' => esc_attr__('Event End Date', 'ptds'),
            'ID' => esc_attr__('Name', 'ptds'),
            //'post_title' => esc_attr__('Name', 'ptds'),
            'activate' => esc_attr__('Activated', 'ptds'),
    ...
    function get_sortable_columns() {
        $sortable_columns = array(
            'EventEndDate' => array('EventEndDate', true), //true means it's already sorted
...
*/
    ...
    /* public function search_box($text, $input_id) {
        $search_by = '';
        if (isset($_REQUEST['s_by']) && $_REQUEST['s_by']) {
            $search_by = $_REQUEST['s_by'];
        }
        ?>
        <p class="search-box">
            <select name="s_by" class="search-by">
                <option value="name" <?php echo selected('name', $search_by); ?>><?php esc_html_e('Search By Name', 'websiteproperties-back-office')?></option>
                <option value="email_address" <?php echo selected('email_address', $search_by); ?>><?php esc_html_e('Search By Email', 'websiteproperties-back-office')?></option>
                <option value="phone" <?php echo selected('phone', $search_by); ?>><?php esc_html_e('Search By Phone', 'websiteproperties-back-office')?></option>
            </select>
...
*/
    ...
    function prepare_items() {
        ...
        // $data = $this->search_box(__('Search Listings'), 'post-search-input');
    ...
    $wp_list_table = new Ptds_dating_activated_Table();  
   
?>


<div class="wrap Activate-dating-listing">    
