...
class Ptds_Admin {
	...
	public function enqueue_scripts($hook) {
		...
		//wp_enqueue_script( $this->plugin_name.'_jquery', plugin_dir_url( __FILE__ ) . '/js/jquery-3.3.1.min.js', array(), '3.3.1', true );
	...
	*/
	...
	public function ptds_dating_extend(){
		...
		$ptdsextenddatechange =  date('Y-m-d', strtotime($ptdsextenddate. ' + '.$ptdsextenddatingdays. 'days'));
		 delete_post_meta($ptdsevent_id, 'dating_meta');
		if ( ! add_post_meta( $ptdsevent_id, 'dating_meta_extend_days', $ptdsextenddatingdays, true ) ) { 
			update_post_meta( $ptdsevent_id, 'dating_meta_extend_days', $ptdsextenddatingdays );
		}

		if ( ! add_post_meta( $ptdsevent_id, 'dating_meta_extend_date', $ptdsextenddatechange, true ) ) { 
			update_post_meta( $ptdsevent_id, 'dating_meta_extend_date', $ptdsextenddatechange );
		}

		if ( ! add_post_meta( $ptdsevent_id, 'dating_meta_extend_date', 'extendeddatingsystemactivate', true ) ) { 
			update_post_meta( $ptdsevent_id, 'extende_dating_system_activated', 'extendeddatingsystemactivate' );
		}
